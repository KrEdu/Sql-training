...
describe("Simple Queries", () => {
  ...
  it(
    ...
    async done => {
      const query = `SELECT strftime('%Y', release_date) AS year, COUNT(strftime('%Y', release_date)) AS count
                      FROM movies
                      GROUP BY year
                      ORDER BY year DESC`;
      const result = await db.selectMultipleRows(query);
  ...
  it(
    ...
    async done => {
      const query = `SELECT strftime('%m', time_created) AS month, COUNT(strftime('%m', time_created)) AS count
                      FROM movie_ratings
                      GROUP BY month
                      ORDER BY count DESC`;
      const result = await db.selectMultipleRows(query);
